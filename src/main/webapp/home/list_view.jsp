...
<body>
<c:if test="${empty LIST }">등록된 게시글이 없습니다.</c:if>
...
<tr><td>이미지</td><td>글제목</td><td>작성자</td>
	<td>작성일</td></tr>
	<c:forEach var="w" items="${LIST }">
	<tr><td><img alt="" src=
	"${pageContext.request.contextPath }/upload/${w.image_name}"
		width="50" height="50"/></td>
		<td><a href="javascript:goView(${w.writing_id })">
			${w.title }</a></td>
		<td>${w.writer_name }</td><td>${w.register_date }</td>
	</tr>
	</c:forEach>
</table>
</c:if>
